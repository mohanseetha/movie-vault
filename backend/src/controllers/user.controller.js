...
export const getRecommendedMovies = async (req, res) => {
  ...
  if (!user || !user.logged.length) return res.json({ recommendations: [] });

  const loggedMovies = Array.from(new Set(user.logged.map(String)));
  const watchlistMovies = new Set(user.watchlist.map(String));
  ...
  for (const movie of loggedDetails) {
    ...
    for (const crew of movie.credits?.crew || []) {
      if (crew.job === "Director")
        favoriteDirectors[crew.name] = (favoriteDirectors[crew.name] || 0) + 1;
    }
